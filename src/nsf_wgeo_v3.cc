...
// declared in res.hh
void wgeo_v3::import_entry(TRANSACT, const std::vector<util::blob> &items)
{
    assert_alive();
    ...
    if (items.size() != 7)
        throw res::import_error("nsf::wgeo_v3: wrong item count");
    ...
    if (vertex_count != item_vertices.size() / 6)
        throw res::import_error("nsf::wgeo_v3: bad vertex item size");
    ...
    if (triangle_count != item_triangles.size() / 6)
        throw res::import_error("nsf::wgeo_v3: bad triangle item size");
    ...
    if (quad_count != item_quads.size() / 8)
        throw res::import_error("nsf::wgeo_v3: bad quad item size");
    ...
    if (item4_count != item_4.size() / 12)
        throw res::import_error("nsf::wgeo_v3: bad item4 size");
    ...
    if (color_count != item_colors.size() / 4)
        throw res::import_error("nsf::wgeo_v3: bad color item size");
    ...
    for (auto &&color : colors) {
        ...
        if (ex != 0)
            throw res::import_error("nsf::wgeo_v3: bad extra color byte");
    }
    r.end();

    // Ensure the item6 count is correct.
    if (item6_count != item_6.size() / 4)
        throw res::import_error("nsf::wgeo_v3: bad item6 size");
    ...
    if (tpag_ref_count > 8)
        throw res::import_error("nsf::wgeo_v3: bad tpag ref count");
...
// declared in nsf.hh
std::vector<util::blob> wgeo_v3::export_entry(uint32_t &out_type) const
{
    assert_alive();
    ...
    if (!world.ok())
        throw res::export_error("nsf::wgeo_v3: bad world ref");

    auto &&model = world->get_model();
    if (!model.ok())
        throw res::export_error("nsf::wgeo_v3: bad model ref");

    auto &&mesh = model->get_mesh();
    if (!mesh.ok())
        throw res::export_error("nsf::wgeo_v3: bad mesh ref");

    auto &&anim = model->get_anim();
    if (!anim.ok())
        throw res::export_error("nsf::wgeo_v3: bad anim ref");

    auto &&frames = anim->get_frames();
    if (frames.size() != 1)
        throw res::export_error("nsf::wgeo_v3: invalid frame count");

    auto &&frame = frames[0];
    if (!frame.ok())
        throw res::export_error("nsf::wgeo_v3: bad frame ref");
    ...
    if (frame->get_x_scale() != 16.0f ||
        ...
        throw res::export_error("nsf::wgeo_v3: scale is invalid");
    }
    ...
    for (auto &&vertex : util::reverse_of(frame->get_vertices())) {
        ...
        if (x < COORD_MIN || x >= COORD_MAX ||
            ...
            throw res::export_error("nsf::wgeo_v3: vertex x/z out of range");
        }

        if (vertex.color_index == -1) {
            throw res::export_error("nsf::wgeo_v3: vertex colors required");
        }
        if (vertex.color_index < 0 || vertex.color_index >= (1L << 10)) {
            throw res::export_error("nsf::wgeo_v3: vertex color out of range");
        }
    ...
    for (auto &&vertex : frame->get_vertices()) {
        ...
        if (y < COORD_MIN || y >= COORD_MAX) {
            throw res::export_error("nsf::wgeo_v3: vertex y out of range");
        }
    ...
    for (auto &&triangle : mesh->get_triangles()) {
        ...
        if (triangle.v[0].color_index != -1 ||
            ...
            throw res::export_error(
                "nsf::wgeo_v3: corner colors not supported"
            );
        }
    }
    w.pad(4);
